 ...
 * recursiva ()
 *
 * Arguments: lp - pointer to list, fp - pointer to file
 * Returns: (void)
 * Side-Effects: writes backwards list on file
 *
 * Description: writes backwards list on file in a recursive manner
 *****************************************************************************/

void recursiva(t_lista *lp, FILE *fp)
{
  if(getProxElementoLista(lp) == NULL)
    return;
  else
  {
  ...
  escreveUmaPalavra ((t_palavra *) getItemLista(lp), fp);
  return;
}
...
int main(int argc, char *argv[])
{
  ...
  if(argc == 3) {
    modo = (char *) malloc (strlen(argv[2]) + 1 );
    strcpy (modo, argv[2]);
  } 

  nomeFicheiroIn = argv[1];
  nomeFicheiroOut = (char *) malloc ( strlen (nomeFicheiroIn) + strlen (extOut) + 1);
  ...
  if ( argc == 2 || ( strcmp (modo, "INICIO") == 0)) {
    aux = lp;
    while(aux != NULL) {
  ...
  else if ( argc == 3 && (strcmp (modo, "FIM") == 0))
  {
    /*recursiva(aux, fpOut);*/
    lp = InverteLista(lp);
    aux = lp;
