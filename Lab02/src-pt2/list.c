 ...
 *
 * Arguments: lp - pointer to list
 * Returns: pointer to inverted list head
 * Side-Effects: inverts list
 *
 * Description: invert list
 *
 *****************************************************************************/

t_lista *InverteLista(t_lista *lp)
{
  t_lista *prev = NULL, *next = NULL;

  while (lp != NULL) {
    next = getProxElementoLista(lp);
    lp -> prox = prev;
    prev = lp;
    lp = next;
  }
...
/*void libertaListafim(t_lista *lp, void freeItem(Item)) {
  t_lista *aux,   auxiliar pointers to travel through the list 
